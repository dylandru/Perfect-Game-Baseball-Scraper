...
async def scrape_all_players(player_id_start: int, player_id_end: int, csv_filename, threads: int = 20, retries: int = 5):
    '''
    ...
    param - retries: integer indicating the amount of times script can retry in getting a player
...
async def scrape_player_by_name(player_name: str, retries: int = 5):
...
async def scrape_players_by_filter(age: int = None, position: str = None, graduation_year: int = None, csv_filename: str = "filtered_pg_players.csv", retries: int = 5):
    '''
    ...
    param - position: string with the abbreviation of position (ex. C = Catcher, 1B = 1st Base, etc) that matches with any postion combo
